...
func ApplyCandidateOrderReversalV2(left, right preprocess.Trajectory, request ApplyRequest) (ApplyV2Outcome, error) {
	if request.Family != FamilyCandidateOrderReversal {
		return ApplyV2Outcome{}, errors.New("pair-order v2 mutation requires candidate_order_reversal family")
	}
	...
	manifest.Program.Operator = operatorForProgram(MutationProgramVersionV2, Definition{Family: request.Family, Operator: legacy.Manifest.Program.Operator})
	manifest.Preservation.BoundaryVersion = EvidenceBoundaryVersionV2
