...
> Historical reference artifact. This file preserves the original Python
> experiment and its reported benchmark tables; it is not the current
> EvalWitness product result or a new empirical claim. For the current product
> contract and admitted bounded evidence, start with the repository
> [README](../../README.md) and [findings report](../../docs/findings.md).
...
For this historical Python path only, create a local, ignored `.env` file with
the required Vertex AI credential. Never commit the file or its contents:
...
.
  README.md
  .env                          # local credential (ignored; never commit)
  scripts/
  ...
  cache/                        # local cached API results (ignored)
